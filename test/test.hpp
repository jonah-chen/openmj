 ...
 */
template <typename T>
void assert_crit(const T &t, const std::string &msg = "Critical Error",
                 int ec = 255)
{
    if (!t)
    {
 ...
 */
FLOATPOINT_T
constexpr bool eq(T a, T b, T eps = 1e-6) { return std::abs(a - b) < eps; }
 ...
 */
EQ_COMPARE_T
constexpr bool eq(T a, T b, double eps = 1e-6) { return a == b; }

EQ_COMPARE_T
void assert_eq(T a, T b, const std::string &msg = "", double eps = 1e-6)
{
    assert(eq(a, b, eps), msg + "|Expected " + std::to_string(a) +
                              " to equal " + std::to_string(b));
}
...
{
    assert(!eq(a, b, eps), msg + "|Expected " + std::to_string(a) +
                               " to not equal " + std::to_string(b));
}

EQ_COMPARE_T
void assert_crit_eq(T a, T b, const std::string &msg = "", double eps = 1e-6)
{
    assert_crit(eq(a, b, eps), msg + "|Expected " + std::to_string(a) +
                                   " to equal " + std::to_string(b));
}
