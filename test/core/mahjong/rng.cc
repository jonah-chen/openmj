...
TEST count_std()
{
    ...
    for (auto i : mj::range1(mj::k_DeckSize - mj::k_DeadWallSize))
        assert(rng_deck(mj::k_East),
               "Deck should have more than " + std::to_string(i) + " tiles");
    assert(!rng_deck(mj::k_East),
           "Deck should have less than " +
               std::to_string(mj::k_DeckSize - mj::k_DeadWallSize + 1) +
               " tiles");

    for (auto i : mj::range1(mj::k_DeadWallSize))
        assert(rng_deck.dora(),
               "Deck should have more than " + std::to_string(i) + " dora");
    assert(!rng_deck.dora(), "Deck should have less than " +
                                 std::to_string(mj::k_DeadWallSize + 1) +
                                 " dora");

    mj::random::RngDeck rng_deck_2{rng};
    for (auto i : mj::range1(mj::k_DeadWallSize))
        assert(rng_deck_2.dora(), "Deck should have more than " +
                                      std::to_string(i) +
                                      " dora when drawn first");
    assert(!rng_deck_2.dora(), "Deck should have less than " +
                                   std::to_string(mj::k_DeadWallSize + 1) +
                                   " dora when drawn first");
    for (auto i : mj::range1(mj::k_DeckSize - mj::k_DeadWallSize))
        assert(rng_deck_2(mj::k_East),
               "Deck should have more than " + std::to_string(i) + " tiles");
    assert(!rng_deck_2(mj::k_East),
           "Deck should have less than " +
               std::to_string(mj::k_DeckSize - mj::k_DeadWallSize + 1) +
...
TEST count_custom_wall()
{
    ...
    for (const auto &[idx, i] : mj::enumerate(check))
    {
        if (idx == 0)
            ...
            assert(i == 2, "For this, tile " + std::to_string(idx) +
                               " should have 2 tiles.");
    }
}
