...
struct _openHand
{
    ...
    {
        assert(h.chii(t, chii_at), "Failed to chii " + std::to_string(t) +
                                       " at " + std::to_string(chii_at));
        return this;
    }
    ...
    void SCORE(mj::Tile agari_pai, mj::U32f score, mj::U64 yakus,
               mj::U64 flags = mj::scoring::f_OpenHandMask &
                               mj::scoring::f_NormalPlay)
    {
...
int main()
{
    OPEN_HAND(m113366s448)
        ...
        ->SCORE(mj::Tile("8s"), 640, 40 | mj::scoring::f_Toitoi);
    OPEN_HAND(m1999p11s11d11)
        ...
        ->SCORE(mj::Tile("1m"), mj::scoring::k_Mangan,
                50 | mj::scoring::f_Honroutou | mj::scoring::f_Toitoi |
                    mj::scoring::f_Hatsu);
    OPEN_HAND(m11133366s4488)
        ->PONG(mj::Tile("6m", mj::k_North))
        ->SCORE(mj::Tile("4s", mj::k_South), 640, 40 | mj::scoring::f_Toitoi);
    OPEN_HAND(m11133366s4488)
        ->PONG(mj::Tile("6m", mj::k_North))
        ->SCORE(mj::Tile("4s", mj::k_East), mj::scoring::k_Mangan,
                50 | mj::scoring::f_Toitoi | mj::scoring::f_Sanankou);
    ...
    OPEN_HAND(m13p13s13d1112)
        ...
        ->SCORE(mj::Tile("2d", mj::k_South), 960,
                30 | mj::scoring::f_ChantaO | mj::scoring::f_SanshokuSeqO |
                    mj::scoring::f_Hatsu);
    ...
    OPEN_HAND(p3355s23568)
        ...
        ->SCORE(mj::Tile("8s", mj::k_South), 240, 30 | mj::scoring::f_Tanyao);
    OPEN_HAND(p3355s23568)
        ->PONG(mj::Tile("3p", mj::k_South))
        ->CHII(mj::Tile("4s", mj::k_North), mj::k_ChiiAbove)
        ->PONG(mj::Tile("5p", mj::k_South))
        ->CHII(mj::Tile("4s", mj::k_North), mj::k_ChiiBelow)
        ->SCORE(mj::Tile("8s", mj::k_South), 0, 0,
                mj::scoring::f_NoOpenTanyaoMask & mj::scoring::f_OpenHandMask &
                    mj::scoring::f_NormalPlay);
