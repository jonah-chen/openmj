...
using namespace mj::ext::tomohxx;
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < 8; i++)
    {
        std::cout << std::setw(4) << (i - 1) << std::setw(12) << sht[i]
                  << std::setw(12) << 100.0 * sht[i] / N << '\n';
        ev += (i - 1) * sht[i];
    }
    ev /= N;
    ...
    std::cout << std::setw(24) << "Time (msec.)" << std::setw(16)
