...
template <int N> using array = std::array<int, N>;
...
template <int N, class Func>
void dealtile(int n, int m, int l, int &cnt, std::vector<array<N>> &vec,
              Func func)
{
    static array<N> hd;
...
template <int N>
void deal(int n, array<10> &kind, std::vector<array<N>> &vec,
          std::ofstream &fout)
{
    static array<N> hd;
    static array<10> sht;
...
void calc(array<N> &t, array<10> &ret, array<10> &kind, Iter itr)
{
    ...
    for (int i = 0; i < 10; i++)
    {
        ...
        {
            tmp = std::inner_product(
                      itr->begin(), itr->end(), t.begin(), 0, std::plus<int>(),
