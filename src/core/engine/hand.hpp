...
public:
    HandElem(const Config &cfg, const HandDense &hand, ImVec2 base=ImVec2(0,0), ImVec2 move_dir=ImVec2(1,0), int rot=0, callback_fn callback=nullptr)
        : cfg_(cfg), hand_(hand), base_(base), move_dir_(move_dir), rot_(rot), callback_(callback) {}
...
public:
    using callback_fn = Config::callback_fn;
public:
    DiscardElem(const Config &cfg, const Discards &discards, ImVec2 base=ImVec2(0,0), ImVec2 dx=ImVec2(1,0), ImVec2 dy=ImVec2(0,1), int rot=0, callback_fn callback=nullptr) 
    :cfg_(cfg), discards_(discards), base_(base), dx_(dx), dy_(dy), rot_(rot), callback_(callback) {}
